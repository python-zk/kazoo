...
class KazooTreeCacheTests(KazooAdaptiveHandlerTestCase):
    ...
    @contextmanager
    def make_cache(self) -> Iterator[TreeCache]:
        self._path = "/" + uuid.uuid4().hex
        assert self.count_tree_node() == 0
        cache = TreeCache(self.client, self.path)
        assert self.count_tree_node() == 1
        cache.listen(lambda event: self._event_queue.put(event))
        cache.listen_fault(lambda error: self._error_queue.put(error))
        cache.start()
    ...
    def test_start(self) -> None:
        with self.make_cache() as cache:
            ...
            assert cache._state == TreeCache.STATE_STARTED
            assert cache._root._state == TreeNode.STATE_LIVE
    ...
    def test_close(self) -> None:
        with self.make_cache() as cache:
            ...
            assert (
                len(self.client._data_watchers[root_path + "/foo/bar/baz"])
                == 1
            )
            assert len(self.client._child_watchers[root_path + "/foo"]) == 2
            assert (
                len(self.client._child_watchers[root_path + "/foo/bar"]) == 1
            )
            assert (
                len(self.client._child_watchers[root_path + "/foo/bar/baz"])
                == 1
            )
            ...
            assert cache._root._children == {}
            assert cache._root._data is None
            assert cache._state == TreeCache.STATE_CLOSED

            # node state should not be changed
            assert cache._root._state != TreeNode.STATE_DEAD
            ...
            assert (
                len(self.client._child_watchers[root_path + "/foo/bar"]) == 0
            )
            assert (
                len(self.client._child_watchers[root_path + "/foo/bar/baz"])
                == 0
    ...
    def test_delete_operation(self) -> None:
        with self.make_cache() as cache:
            ...
            assert (
                self.client._data_watchers[root_path + "/foo/bar/baz"] == set()
            )
            assert self.client._child_watchers[root_path + "/foo"] == set()
            assert self.client._child_watchers[root_path + "/foo/bar"] == set()
            assert (
                self.client._child_watchers[root_path + "/foo/bar/baz"]
                == set()
            )

            # This should be the only tree left
            assert self.count_tree_node() == 1
    ...
    def test_children_operation(self) -> None:
        with self.make_cache():
            ...
            self.client.create(
                self.path + "/test_children", b"test_children_1"
            )
    ...
    def test_get_children(self) -> None:
        with self.make_cache() as cache:
            ...
            with patch.object(cache, "_client"):
                assert (
                    cache.get_children(self.path + "/foo/bar/baz")
                    == frozenset()
                )
                assert cache.get_children(self.path + "/foo/bar") == frozenset(
                    ["baz"]
                )
                assert cache.get_children(self.path + "/foo") == frozenset(
                    ["bar"]
                )
    ...
    def test_get_data_out_of_tree(self) -> None:
        with self.make_cache() as cache:
            ...
            with pytest.raises(ValueError):
                cache.get_data("/out_of_tree")

    def test_get_children_out_of_tree(self) -> None:
        with self.make_cache() as cache:
            self.wait_cache(since=TreeEvent.INITIALIZED)
            with pytest.raises(ValueError):
                cache.get_children("/out_of_tree")
        ...
        with self.make_cache() as cache:
            ...
            with self.spy_client("get_async") as get_data:
                with self.spy_client("get_children_async") as get_children:
                    ...
                    # There are a serial refreshing operation here. But
                    # NODE_ADDED events will not be raised because the zxid of
                    # nodes are the same during reconnecting.

                    # connection restore
                    self.wait_cache(TreeEvent.CONNECTION_RECONNECTED)

                    # wait for outstanding operations
                    while cache._outstanding_ops > 0:
                        self.client.handler.sleep_func(0.1)

                    # inspect in-memory nodes
                    _node_root = cache._root
                    _node_foo = cache._root._children["foo"]
                    ...
                    get_data.assert_has_calls(
                        [
                            ...
                            call(
                                self.path + "/foo",
                                watch=_node_foo._process_watch,
                            ),
                        ],
                        any_order=True,
                    )
    ...
    def test_root_recreated(self) -> None:
        with self.make_cache() as cache:
            ...
            assert cache._outstanding_ops >= 0, (
                "unexpected outstanding ops %r" % cache._outstanding_ops
            )
    ...
    def test_exception_handler(self) -> None:
        ...
        with patch.object(TreeNode, "on_deleted") as on_deleted:
            ...
            with self.make_cache() as cache:
                cache.listen_fault(error_handler)
    ...
    def test_exception_suppressed(self) -> None:
        with self.make_cache() as cache:
            ...
            with patch.object(TreeNode, "on_created") as on_created:
                cache._root._call_client("exists", "/")
                cache._root._call_client("get", "/")
                cache._root._call_client("get_children", "/")
